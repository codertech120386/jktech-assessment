...
from .service import create_book_in_db, fetch_book_by_id_from_db, delete_book_in_db, update_book_in_db, \
    ...
    fetch_summary_and_avg_ratings_of_book_by_id_from_db


def add_book(book: CreateBookSchema, session: SessionLocal):
    try:
        book = create_book_in_db(title=book.title, author=book.author, genre=book.genre,
                                 year_published=book.year_published, session=session)
...
def update_book(book_id: int, book: UpdateBookSchema, session: SessionLocal):
    try:
        ...
        updated_book = update_book_in_db(book_id=book_id, title=book.title, author=book.author,
                                         genre=book.genre,
...
def delete_book(book_id, session):
    try:
        fetched_book = fetch_book_by_id_from_db(book_id=book_id, session=session)
        if not fetched_book:
            return generic_response(
...
def fetch_books(active_page, session):
    try:
...
def fetch_book_by_id(book_id, session):
    try:
...
def fetch_reviews_by_book_id(active_page, book_id, session):
    try:
...
def fetch_book_summary_and_aggregate_ratings_by_id(book_id, session):
    try:
        book = fetch_book_by_id(book_id=book_id, session=session)
        if not book:
            return generic_response(
                {"error": True, "message": "Book does not exist", "status_code": status.HTTP_400_BAD_REQUEST})
